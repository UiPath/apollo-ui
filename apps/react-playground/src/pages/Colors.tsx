...
export function Colors() {
	...
	const filteredSemanticGroups = !searchTerm
		...
		: Object.entries(semanticGroups)
				.map(([groupName, variants]) => {
					...
					const filteredVariants = Object.entries(variants).filter(
						([variant, value]) => {
				...
				.filter(
					([, variants]) =>
						Object.keys(variants as Record<string, string>).length > 0,
				); // Only keep groups with matching variants
	...
	const filteredSemanticCount = filteredSemanticGroups.reduce(
		(acc, [, variants]) =>
			acc + Object.keys(variants as Record<string, string>).length,
		0,
	);
		...
		<PageContainer>
			...
			{searchTerm && filteredSemanticCount === 0 && filteredBaseCount === 0 && (
				<NoResultsContainer>
					...
					<NoResultsText>
						Try searching for a different color name or hex value
					</NoResultsText>
				</NoResultsContainer>
			)}
					...
					<section>
						...
						<SectionContainer>
							{filteredSemanticGroups
								...
								.map(([groupName, variants]) => {
									const sortedVariants = Object.entries(
				...
				{filteredBaseCount > 0 && (
					<section>
						...
						>
							...
							{searchTerm
								? `${filteredBaseCount}/${baseColors.length}`
								: filteredBaseCount}{" "}
							token
									...
									<ColorGroup key={category as string}>
										<GroupHeader>
											...
											<GroupCount>
												...
												{categoryColors?.length && categoryColors.length > 1
													? "s"
													: ""}
											</GroupCount>
										</GroupHeader>
										...
										<ColorGrid $columns="repeat(auto-fit, minmax(200px, 1fr))">
											{(categoryColors as { name: string; value: string }[])!
												...
												.map((color) =>
													renderColorCard(color.name, color.value),
												)}
										</ColorGrid>
									</ColorGroup>
								))}
