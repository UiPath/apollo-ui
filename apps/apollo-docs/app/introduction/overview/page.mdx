...
| [Apollo Vertex](https://engdogfood.staging.uipath.host/apollo-vertex/) | Complete design system showcase and component registry |

---

## Features

- **Design Tokens** — comprehensive color system, typography, spacing
- **Icons** — 1300+ SVG icons in `apollo-ui-icons`
...
apollo-ui-icons  (SVG Icon Set)
     ↑
     ├── apollo-react
     └── apollo-core  (re-export only, deprecated)
```

- **apollo-core** is the foundation — all other packages depend on it
- **apollo-ui-icons** holds the icon set; `apollo-core/icons` still re-exports it but is deprecated
...
Design tokens and fonts. The framework-agnostic foundation for the entire design system. Both `apollo-react` and `apollo-wind` depend on this package.

### `@uipath/apollo-ui-icons`

1300+ SVG icons, importable as modules or raw assets. `@uipath/apollo-core/icons` still re-exports these for backwards compatibility but is deprecated and will be removed in the next major.
