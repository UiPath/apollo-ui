...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { className: string }> = {
    ...
    Pending: {
      className:
        "bg-white/50 text-slate-600 border-slate-200/40 shadow-[0_0_10px_-2px_rgba(185,205,230,0.15),inset_0_1px_4px_0_rgba(255,255,255,0.35)]",
    },
    "In Review": {
      className:
        "bg-white/50 text-slate-600 border-slate-200/40 shadow-[0_0_10px_-2px_rgba(185,205,230,0.15),inset_0_1px_4px_0_rgba(255,255,255,0.35)]",
    ...
    Failed: {
      className:
        "bg-rose-100/50 text-rose-600 border-rose-200/40 shadow-[0_0_10px_-2px_rgba(244,63,94,0.10),inset_0_1px_4px_0_rgba(255,255,255,0.4)]",
    },
  };
  ...
  return (
    <div className={`${glassCard} p-5 flex flex-col gap-3`}>
      <div className="flex items-center justify-between">
        ...
        <Icon className="size-4 text-slate-400 stroke-[1.5]" />
      </div>
      <div className="text-2xl font-bold text-slate-700 tracking-tight">
        {value}
      </div>
      <span
        className={`text-xs font-medium ${isPositive ? "text-emerald-600/80" : "text-rose-500/80"}`}
      >
...
const glassNavActive = "text-primary font-semibold";
  ...
  return (
    <aside className="w-[220px] shrink-0 flex flex-col py-5 px-3 gap-1 bg-white/50 backdrop-blur-[20px] border-r border-white/60">
      ...
      <div className="flex items-center gap-2.5 px-2 mb-6">
        <div className={`size-9 rounded-xl flex items-center justify-center bg-slate-800/90 backdrop-blur-[10px] border border-white/10 shadow-[0_0_20px_-2px_rgba(30,40,60,0.25),inset_0_1px_8px_0_rgba(255,255,255,0.08)]`}>
        ...
        <div className="flex flex-col leading-none">
          ...
          <span className="text-[11px] text-slate-400">Apollo Vertex</span>
        </div>
      </div>
        ...
        {navItems.map((item) => (
          <div
            ...
            className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm transition-all cursor-pointer ${
              item.active
                ? glassNavActive
                : "text-slate-500 hover:bg-white/35 hover:text-slate-600 hover:shadow-[0_0_12px_-2px_rgba(200,215,235,0.2)]"
            }`}
          >
      ...
      <div className="mt-auto pt-4">
        <div className="flex items-center gap-2.5 px-2">
          <div className="size-8 rounded-full bg-white/50 backdrop-blur-[10px] border border-white/50 shadow-[0_0_12px_-2px_rgba(185,205,230,0.25),inset_0_1px_6px_0_rgba(255,255,255,0.35)] flex items-center justify-center text-xs font-semibold text-slate-500">
            DU
          </div>
          <div className="flex flex-col leading-tight">
            ...
            <span className="text-[11px] text-slate-400">dev@localhost</span>
          </div>
        </div>
      </div>
    </aside>
...
export function HaidyThemeDashboard() {
  return (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-300/25">
                  <th className="text-left font-medium text-slate-500 px-5 py-2.5">
                    Invoice
                  </th>
                  <th className="text-left font-medium text-slate-500 px-5 py-2.5">
                    Vendor
              ...
              <tbody>
                {invoices.map((inv) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-slate-500">{inv.vendor}</td>
            ...
            >
              <BarChart data={chartData}>
                ...
                <Bar
                  ...
                  radius={[6, 6, 0, 0]}
                  opacity={0.8}
                  activeBar={{ filter: "url(#tealGlow)", opacity: 1 }}
                />
              </BarChart>
            </ChartContainer>
          </div>
