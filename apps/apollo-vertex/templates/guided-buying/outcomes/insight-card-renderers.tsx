...
import { AreaContent, DonutContent, SparklineContent } from "./chart-stubs";
...
function HorizontalBarsContent({
  ...
  // One color across the series (prompt 62 step 5): length carries the
  // comparison here, so a color cycle would make the longest bar read as
  // a warning rather than simply the largest value.
  const barColor = "bg-chart-1";
  const barsWithColor = bars.map((b) => ({ ...b, color: barColor }));
      ...
      {barsWithColor.map((issue) => (
        <div key={issue.label}>
          <div className="flex items-center justify-between text-xs mb-1.5">
            ...
            <span className="font-bold">{issue.value} days</span>
          </div>
          <div className="h-1 w-full rounded-full bg-muted dark:bg-foreground/10 relative">
            <div
              ...
              style={{
                width: `${maxValue > 0 ? (issue.value / maxValue) * 100 : 0}%`,
              }}
            >
              <div
