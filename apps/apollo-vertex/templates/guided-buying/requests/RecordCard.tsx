...
/** The AI caveat — same icon, text, and muted treatment wherever it
 * renders, in either placement (`"ai-zone"` or `"footer"`, see
 * RecordCard), so the two can't drift into two different caveats. */
function Caveat({ className }: { className?: string }) {
  return (
    <p
      className={cn(
  ...
  const footer = showFooter && (
    <>
      ...
      <div className="flex items-center justify-between gap-2 p-5">
        ...
        {caveatPlacement === "footer" && aiContent != null && <Caveat />}
      </div>
    </>
  );
