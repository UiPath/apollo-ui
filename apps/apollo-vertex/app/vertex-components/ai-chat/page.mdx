...
- **Type-Safe Display Tools** — `AiChat<TTools>` is generic over your tools — renderer keys are constrained to actual tool names, and render function args are inferred from each tool's `inputSchema`
...
Display tools render UI inline in the chat when the LLM calls them. `AiChat<TTools>` is generic — pass `ToolRenderers<typeof toolDefs>` and TypeScript enforces that renderer keys match tool names and render args match each tool's `inputSchema`.
...
import { stream, useChat } from '@tanstack/ai-react';
import { AiChat } from '@/components/ui/ai-chat/components/ai-chat';
import type { ToolRenderers } from '@/components/ui/ai-chat/types';
...
// 2. Define renderers — keys and args are type-checked from toolDefs
const toolRenderers: ToolRenderers<typeof toolDefs> = {
  show_results: ({ entityName, columns }) => (
    // entityName is string, columns is string[] — fully typed!
    <ResultsTable entity={entityName} columns={columns} />
  ),
  // typo_tool: () => ...  ← TypeScript ERROR: not a valid tool name
};
...
**How it works:**
- `ToolRenderers<typeof toolDefs>` is a mapped type — each key is a tool name, each value is a render function with args inferred from that tool's `inputSchema`
- TypeScript catches typos in tool names and wrong arg types at compile time
- At runtime, `AiChatMessage` uses `tc.input` (pre-parsed by TanStack AI) with a `JSON.parse(tc.arguments)` fallback
...
Generic chat UI component. `TTools` is inferred from `toolRenderers` — pass `ToolRenderers<typeof toolDefs>` for full type safety.
...
| `toolRenderers` | `ToolRenderers<TTools>` | — | Typed map of tool names to render functions |
...
### `ToolRenderers<TTools>`

Mapped type that constrains renderer keys to actual tool names and infers render function args from each tool's `inputSchema`.

```tsx
import type { ToolRenderers } from '@/components/ui/ai-chat/types';

// Keys and args are fully typed from toolDefs
const renderers: ToolRenderers<typeof toolDefs> = {
  show_results: ({ entityName, columns }) => <ResultsTable ... />,
};
```
...
// Tool renderers (generic mapped type)
import type { ToolRenderers } from '@/components/ui/ai-chat/types';
