...
> **Try it out** — type *"give me some choices"* in the demo above to see suggestion buttons in action. The demo uses the `presentChoices` tool defined in `tools/choices/choices-tool.tsx`.
...
Choices are rendered via `toolsRenderer` on `AiChatMessage`. The render function receives the tool output and calls `sendMessage` when a choice is selected:
...
<AiChat messages={messages} isLoading={isLoading} onSendMessage={sendMessage} onStop={stop}>
  {messages.map((message) => (
    <AiChatMessage key={message.id} message={message} toolsRenderer={toolsRenderer} />
  ))}
</AiChat>
```
