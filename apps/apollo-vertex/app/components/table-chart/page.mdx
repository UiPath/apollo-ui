import {
  TableChartLoadingTemplate,
  TableChartTemplate,
} from '@/templates/charts/TableChartTemplate';
...
`TableChartWithAdapter` wraps refetching views with `SpinnerWithChildren`. Content is dimmed with `opacity-30` and pointer events are blocked so sort and other controls do not fire.
