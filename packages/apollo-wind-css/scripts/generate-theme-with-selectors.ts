...
// Find apollo-core in node_modules
const APOLLO_CORE_PATH = path.resolve(__dirname, '../node_modules/@uipath/apollo-core/src');
...
function generateThemeCSS(
  ...
  for (const colorName of sortedColors) {
    ...
    if (!variableName) {
      console.warn(`Warning: No variable name for ${colorName}`);
      continue;
    }
    try {
      let value = resolveVariable(variableName, variablesContent);
      // Remove SCSS !default flag
 ...
 */
function main() {
  console.log('🎨 Generating theme CSS with body selectors...');

  const themePath = path.join(APOLLO_CORE_PATH, 'scss/theme.scss');
  const variablesPath = path.join(APOLLO_CORE_PATH, 'scss/_variables.scss');
  ...
  css.push(' * ');
  css.push(' * NOTE: This file is auto-generated but COMMITTED TO GIT for developer visibility.');
  css.push(
    ' * Developers can read this file to understand Apollo theme mappings without running the build.'
  );
  css.push(' * Regenerate by running: yarn generate:theme-selectors');
  css.push(' * ');
  ...
  const outputPath = path.resolve(__dirname, '../src/generated/theme-selectors.css');
  ...
  console.log(`   File: src/generated/theme-selectors.css`);
