...
export const PrimitiveValueSchema = z.union([z.string(), z.boolean(), z.number().nullable()]);
...
const DataQueryResponseMetricSchema = z.object({
  ...
  ungrouped: z.union([z.number().nullable(), z.array(z.number().nullable())]).default(null),
  values: z.array(DataQueryResponseEntrySchema).nullable(),
});

export const DataQueryResponseSchema = z.record(z.string(), DataQueryResponseMetricSchema);
