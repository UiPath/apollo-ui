...
import type { FeatureFlagProviderConfig } from './types';

export const FeatureFlagContext = createContext<FeatureFlagProviderConfig | null>(null);

export function useFeatureFlagProvider(): FeatureFlagProviderConfig {
  const provider = use(FeatureFlagContext);
  if (!provider) {
    throw new Error('useFeatureFlagProvider must be used within a <FeatureFlagProvider>.');
  }
  return provider;
}
