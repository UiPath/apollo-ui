...
import { EvaluatorResultsView, parseEvaluatorResults } from './evaluator-results-view';
...
export const ResultExpandedContent = ({ result, data }: ResultExpandedContentProps) => {
  ...
  const isPassedOrFailed = status === RunResultStatus.Passed || status === RunResultStatus.Failed;
  const evaluatorResults = parseEvaluatorResults(data.evaluatorResults);
  const hasEvaluatorResults = evaluatorResults != null && Object.keys(evaluatorResults).length > 0;
      ...
      {status === RunResultStatus.NoBaseline &&
        (data.actual == null ? (
          <div>
            ...
            <p className="text-sm text-muted-foreground">{t('agent_produced_no_output')}</p>
          </div>
        ) : (
          <ProcessOutputView
          ...
          {result.ErrorMessage && (
            <div>
              ...
              <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3">
                <pre className="whitespace-pre-wrap break-words text-xs">{result.ErrorMessage}</pre>
              </div>
            </div>
          )}
          {data.actual != null && (
