...
export const ProcessOutputView = ({
  ...
  const renderer = resolveProcessOutputRenderer({ agentId, processName }, outputRenderers);
  const rendered = renderer?.({ output }) ?? null;

  if (rendered == null) {
    if (variant === 'bare') {
      return <pre className="whitespace-pre-wrap break-words text-xs">{formatJson(output)}</pre>;
    }
