...
import { FieldCell, FieldGroupHeading, FieldRow, FieldTable } from '../../ixp/shared/field-table';
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        {documents.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t('ixp_output_no_documents')}</p>
        ) : (
          documents.map(([fileKey, doc]) => <DocumentExtractionRow key={fileKey} doc={doc} />)
        )}
      </div>
    </div>
  );
...
const OutputSummary = ({ output, documentCount }: { output: IxpOutput; documentCount: number }) => {
  const { t } = useTranslation();
  const parts = [t('ixp_output_total_files', { count: output.total_files ?? documentCount })];
  ...
  if (output.unclassified_count != null) {
    parts.push(t('ixp_output_unclassified', { count: output.unclassified_count }));
  }
...
const DocumentExtractionRow = ({ doc }: { doc: IxpDocumentExtraction }) => {
  const { t } = useTranslation();
  const fieldCount = doc.extractions.reduce((total, group) => total + group.fieldValues.length, 0);
...
const ExtractionGroup = ({ group }: { group: IxpExtractionGroup }) => {
  ...
  return (
    <div className="mt-3 first:mt-0">
      ...
      <FieldTable columns={[{ label: t('ixp_field') }, { label: t('ixp_value') }]}>
...
export const IXP_OUTPUT_RENDERER = makeProcessOutputRenderer(IxpOutputSchema, IxpOutputResult, {});
