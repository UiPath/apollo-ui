...
export type GenericEvaluatorDetails = z.infer<typeof GenericEvaluatorDetailsSchema>;

function scoreColorClass(score: number | undefined, passThreshold: number): string {
    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-md border bg-muted/50 p-3">
        <div className="flex items-center justify-between">
          ...
          <span className={`text-sm font-semibold ${scoreColorClass(score, passThreshold)}`}>
            {scoreStr}
          </span>
        </div>
        {justification && <p className="mt-2 whitespace-pre-wrap text-xs">{justification}</p>}
      </div>
