...
export const IxpExtractionResult = ({ evaluatorDetails }: EvaluatorResultProps<IxpDetails>) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <ProvenanceBar expected={expected_provenance} actual={actual_provenance} />
      <SummaryBar
      ...
      <div className="flex flex-col gap-2">
        {documents.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t('ixp_no_documents')}</p>
        ) : (
          documents.map((doc, i) => (
