...
import { defaultRunResultStatusLabels, defaultRunStatusLabels } from './constants';
...
import { resultBadgeClassMap, runResultStatusBadgeMap, runStatusBadgeMap } from './status-maps';
import { RunResultStatus, type SolutionTestRun, type SolutionTestRunResult } from './types';
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border px-4 py-3">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2">
            <h3 className="text-base font-semibold text-foreground">{result.ProcessName}</h3>
            <Badge
              variant="secondary"
...
const BaselineStateLabel = ({
  ...
  if (isBaselineSource) {
    return <span className="text-xs font-medium text-info">{t('current_baseline')}</span>;
  }
  if (inBaseline) {
    return (
      <span className="text-xs font-medium text-muted-foreground">{`✓ ${t('in_baseline')}`}</span>
    );
  }
  if (hadBaseline) {
    return <span className="text-xs font-medium text-destructive">{t('removed')}</span>;
  }
  return null;
};
