...
import type { ColumnDefWithAccessorKey, EntityRecord, ExtraColumn, VssEntity } from './types';
...
export interface UseEntityDataTableOptions<TRecord extends EntityRecord = EntityRecord> {
  entity: VssEntity;
  storageKey?: string;
  systemFields?: string[];
  columnOrder?: string[];
...
export function useEntityDataTable<TRecord extends EntityRecord = EntityRecord>({
  entity,
  ...
  const { data = [], isLoading } = useLiveQuery((q) => q.from({ entities: collection }));
  ...
  const defaultVisibleColumns =
    columnOrderProp && columnOrderProp.length > 0
      ? columnOrderProp.filter((key) => allColumns.some((col) => col.key === key))
