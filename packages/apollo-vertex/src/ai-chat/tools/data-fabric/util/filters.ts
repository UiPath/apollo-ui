...
const stringListFilterSchema = z.object({
  ...
  invert: z.boolean().optional().describe('If true, exclude matching values instead'),
});
...
const searchFilterSchema = z.object({
  ...
  searchFilterType: z.enum(['default', 'startsWith', 'endsWith']).describe('default = contains'),
});
...
const datetimeRangeFilterSchema = z.object({
  ...
  range: z.object({
    min: z.string().describe('ISO 8601 datetime, inclusive lower bound (e.g. 2026-01-01)'),
    max: z.string().describe('ISO 8601 datetime, inclusive upper bound (e.g. 2026-12-31)'),
    inclusive: z.boolean().optional(),
  }),
});
...
export type ResolvedFilter = NonNullable<TableChartConfiguration['filters']>[number];
...
function fieldResolver(options: ResolveFiltersOptions): (field: string) => string | null {
