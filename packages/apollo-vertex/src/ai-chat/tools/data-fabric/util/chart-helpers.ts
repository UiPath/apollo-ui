...
export function buildDataModel<T extends DataModelFieldType>({
  ...
  dimensionType,
  metric,
}: BuildDataModelInput<T>): ChartDataModel<Extract<DataModelField, { type: T }>> {
  return {
    id,
...
export function buildMultiMetricDataModel<T extends DataModelFieldType>({
  ...
  dimensionType,
  metrics,
}: BuildMultiMetricDataModelInput<T>): ChartDataModel<Extract<DataModelField, { type: T }>> {
  return {
    id,
    dimensions: [buildField(dimension, dimensionType)],
  ...
  .discriminatedUnion('aggregation', [
    z.object({
      ...
      field: z
        .string()
        .optional()
        .describe('Field counted per bin. Optional — defaults to the entity primary key.'),
    }),
    z.object({
      aggregation: z.enum(['SUM', 'AVG', 'MIN', 'MAX']),
      field: z
        .string()
        .describe('Numeric field aggregated by the metric. Required for SUM/AVG/MIN/MAX.'),
    }),
  ])
  .describe('Metric to plot per bin. Omit entirely for the default COUNT of records.');
...
export interface ResolvedDimension<T extends DataModelFieldType = DataModelFieldType> {
  id: string;
  type: T;
}
...
export function resolveMultiDimension<T extends DataModelFieldType>(
  ...
  const qualified = dimension.includes('.') ? dimension : `${primaryEntity}.${dimension}`;
...
export function resolveSingleMetric(
  ...
  if (!metric || metric.aggregation === 'COUNT') {
    const userField =
      metric?.field && entity.fields.some((f) => f.name === metric.field) ? metric.field : null;
...
export function resolveMultiMetric(
  ...
  if (!metric || metric.aggregation === 'COUNT') {
    ...
    const fieldName = userField ?? pickCountFieldQualified(primaryEntity, qualifiedFields);
    if (!fieldName) {
      return fail({
    ...
    return ok({
      argument: buildField(fieldName, type),
      aggregation: 'COUNT',
      display: userField ? `Count of ${unqualifiedDisplay(userField)}` : 'Count',
    });
  }
  ...
  const qualified = metric.field.includes('.') ? metric.field : `${primaryEntity}.${metric.field}`;
  const field = qualifiedFields.get(qualified);
  if (!field) {
    return fail({
      reason: 'unknown_field_in_joined',
