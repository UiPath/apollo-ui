...
export function AiChatHeader({
  ...
  const copyLabel = error ? t('copy_conversation_failed') : t('copy_conversation');

  return (
    <div className="relative z-10 py-3 px-4 flex items-center justify-between gap-2 bg-background">
      <div className="flex items-center gap-1.5 min-w-0">
        <AutopilotGradientIcon size={21} className="shrink-0" aria-hidden="true" />
      ...
      {hasMessages && (
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>{t('new_conversation_confirm_title')}</AlertDialogTitle>
              <AlertDialogDescription>
