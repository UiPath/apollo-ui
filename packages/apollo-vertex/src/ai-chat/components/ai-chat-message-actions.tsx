...
function IconActionButton({ icon, label, onClick, pressed }: IconActionButtonProps) {
  return (
    <TooltipPrimitive.Root>
      <TooltipTrigger asChild>
        <Button
          ...
          onClick={onClick}
          className={cn('hover:bg-ai-chat-muted', pressed && 'bg-ai-chat-muted text-foreground')}
        >
          {icon}
        </Button>
      </TooltipTrigger>
    ...
    <IconActionButton
      icon={
        copied ? (
          ...
          <Copy className="size-3.5 text-ai-chat-muted-foreground" aria-hidden="true" />
        )
      }
      label={label}
      onClick={() => copy(value)}
