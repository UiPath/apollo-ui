...
const components: ComponentProps<typeof ReactMarkdown>['components'] = {
  p: ({ children }: NodeProps) => <p className="mt-5 mb-5 first:mt-0 last:mb-0">{children}</p>,
  ul: ({ children }: NodeProps) => (
    <ul className="mt-5 mb-5 first:mt-0 last:mb-0 ml-4 list-disc">{children}</ul>
  ),
  ol: ({ children }: NodeProps) => (
    <ol className="mt-5 mb-5 first:mt-0 last:mb-0 ml-4 list-decimal">{children}</ol>
  ),
  li: ({ children }: NodeProps) => <li className="mb-3">{children}</li>,
  pre: ({ children }: NodeProps) => <div>{children}</div>,
  code: ({ children, className, ...props }: NodeProps & { className?: string }) => {
    const isBlock =
  ...
  table: ({ children }: NodeProps) => (
    <div className="overflow-x-auto my-2">
      <table className="min-w-full divide-y divide-ai-chat-border">{children}</table>
    </div>
  ),
  thead: ({ children }: NodeProps) => <thead className="bg-ai-chat-muted">{children}</thead>,
  ...
  td: ({ children }: NodeProps) => <td className="px-3 py-2 text-xs">{children}</td>,
};
