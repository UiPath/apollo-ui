...
export function AiChatImagePreview({ url, onClose }: AiChatImagePreviewProps) {
  ...
  return (
    <Dialog open={!!url} onOpenChange={(open) => !open && onClose()}>
      <DialogContent fullscreen className="bg-black/80 border-0 flex items-center justify-center">
        <DialogTitle className="sr-only">{t('image_preview')}</DialogTitle>
        <DialogDescription className="sr-only">{t('image_preview')}</DialogDescription>
        {url && (
          <img
            src={url}
            alt={t('image_preview')}
