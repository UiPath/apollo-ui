 ...
 */
function ConfidenceSignalCta({ cta, className }: { cta: ConfidenceCta; className?: string }) {
  const content = (
    <>
      {cta.label}
      {/* Decorative: the label already says where the CTA goes. */}
  ...
  return (
    <Button variant="outline" size="sm" className={cn('w-full', className)} onClick={cta.onClick}>
      {content}
    </Button>
  );
}
