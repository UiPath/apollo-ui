...
function KpiChartResolver({ configuration, dataModel, dataAdapter }: KpiChartWithAdapterProps) {
  const {
    i18n: { language },
  } = useTranslation();
  ...
  const metric = metricId ? dataModel.metrics.find((m) => m.id === metricId) : null;

  const rawValue = Array.isArray(data.data) ? data.data[0] : data.data;
  const numericValue = rawValue == null ? 0 : assertNumber(rawValue, 'KPI numeric value');
  const valueText = metric ? formatMetricValue(language, numericValue, metric.expression) : '';
