...
export function AiCaveat({ variant = 'standalone', children, className }: AiCaveatProps) {
  ...
  if (variant === 'withMark') {
    return (
      <span className={cn('block pl-5 text-xs text-muted-foreground', className)}>
  ...
  return (
    <span className={cn('mt-4 flex items-start gap-1.5 text-xs text-muted-foreground', className)}>
