...
import { exportCanvasToImage, downloadCanvasAsImage, copyCanvasToClipboard, getFileExtension, getMimeType } from "./export-canvas";
...
describe("export-canvas utilities", () => {
  ...
  describe("exportCanvasToImage", () => {
    it("throws error when no nodes are found", async () => {
      ...
      await expect(exportCanvasToImage(emptyInstance)).rejects.toThrow("Cannot export canvas: no nodes found");
    });

    it("throws error when React Flow wrapper is not found", async () => {
      vi.spyOn(document, "querySelector").mockReturnValue(null);

      await expect(exportCanvasToImage(mockReactFlowInstance)).rejects.toThrow("Cannot find React Flow wrapper element");
    });
    ...
    it("throws error when viewport element is not found", async () => {
      ...
      await expect(exportCanvasToImage(mockReactFlowInstance)).rejects.toThrow("Cannot find viewport element with selector");
    });
  ...
  describe("copyCanvasToClipboard", () => {
    ...
    it("throws error when Clipboard API is not available", async () => {
      ...
      await expect(copyCanvasToClipboard(mockReactFlowInstance)).rejects.toThrow("Clipboard API is not supported in this browser");
    });
