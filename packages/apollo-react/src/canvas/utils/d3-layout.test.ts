...
describe("d3-layout", () => {
  describe("d3HierarchyLayout", () => {
    ...
    it("handles spacing parameter correctly", async () => {
      ...
      const smallDistance = Math.abs((smallChild1?.position.x || 0) - (smallChild2?.position.x || 0));
      ...
      const largeDistance = Math.abs((largeChild1?.position.x || 0) - (largeChild2?.position.x || 0));
