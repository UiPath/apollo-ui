...
/**
 * Every connector with a slot whose target is `nodeId`. One definition of "what
 * counts as an incoming seam", so callers that want the FIRST one
 * ({@link ownIncomingConnector}) and callers that want to know whether there is
 * MORE than one (`hasMultipleIncomingSeams`, slotNavigation.ts) cannot drift
 ...
 * owner, see below).
 */
export function incomingSeams(projection: SequenceProjection, nodeId: string): SequenceConnector[] {
  return projection.connectors.filter((c) => c.targetRowId === nodeId && c.slot);
}
...
export function ownIncomingConnector(
  ...
  return incomingSeams(projection, nodeId)[0];
}
