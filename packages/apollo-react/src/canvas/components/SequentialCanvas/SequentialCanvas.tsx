...
/**
 * Splits a node's visible, non-artifact source handles into the ONE continuation
 * (spine) output and the branch lanes, returned here in manifest order. Exported
 * as a pure helper so the precedence below is testable without mounting a canvas.
 *
 * Precedence for the continuation:
 *   1. the registry's EXPLICIT default source handle (`isDefaultForType`), the
 *      only authoritative statement a manifest makes about which output carries
 *      the forward flow -- this is what lets a spine named `next` / `then` /
 *      `done` / `out` keep its place on the spine;
 *   2. a well-known `output` / `success` id, for manifests that flag nothing;
 *   3. the sole candidate, when the node has exactly one source.
 *
 * The registry default is honoured only when it is itself one of `candidates`: a
 * default pointing at a hidden handle, an artifact handle, or an unexpanded
 * `repeat` template id must not leave EVERY real handle classified as a lane,
 * which would indent a node's whole forward flow under itself.
 *
 * Note the caller passes the default id only when the manifest actually set
 * `isDefaultForType`. `getDefaultHandle`'s positional fallback (first source
 * handle when nothing is flagged) is deliberately NOT accepted: control-flow
 * manifests flag no default (If → true/false, switch → cases), so the fallback
 * would silently promote the FIRST lane ("True") to the spine and drop it as a
 * lane. `isDefaultForType` is likewise not a branch discriminator in the other
 * direction -- it describes default connection selection, not control-flow
 * semantics, so its ABSENCE never means "this handle is a branch".
 */
export function resolveBranchHandleIds(
  candidates: readonly { id: string }[],
  registryDefaultSourceHandleId: string | undefined
): string[] {
  // One `find` per documented rule, in precedence order, so the code reads as the
  // same three-item list the doc above states. Rule 1 needs no separate "is it a
  // candidate" test: a `registryDefaultSourceHandleId` that is absent (or
  // `undefined`) simply matches nothing and falls through, which is exactly the
  // guard described above.
  const continuationId =
    candidates.find((handle) => handle.id === registryDefaultSourceHandleId)?.id ??
    candidates.find((handle) => handle.id === 'output' || handle.id === 'success')?.id ??
    (candidates.length === 1 ? candidates[0]?.id : undefined);
  return candidates.filter((handle) => handle.id !== continuationId).map((handle) => handle.id);
}
 ...
 * mounted BaseCanvas, selected by the orthogonal `view` prop (see
 * SequentialCanvasStoryHarness in canvas/storybook-utils/sequential, which drives
 * the "Interactive Sequence" story, and SequentialViewProvider for the persisted
 * variant). Mutations flow out through the standard
...
function SequentialCanvasInner<N extends Node, E extends Edge>({
  ...
  // Known tradeoff, deliberately not optimized: this re-resolves `resolveHandles`
  // for EVERY node whenever the `nodes` array identity changes -- including a
  // single rename keystroke, which produces a fresh array. That churns the
  // identity of `getBranchHandles` / `resolveBranchLabel`, which in turn re-runs
  // several O(n) fingerprint string-building passes in useSequentialGraph. The
  // structural projection itself is correctly NOT recomputed (that is exactly what
  // the D12 fingerprint buys), but the cost of PROVING it is O(n) per keystroke.
  // Acceptable below the SEQ_FULL_RENDER_MAX_NODES (150) ceiling; if the
  // sequential view ever feels sluggish while typing, profile this first (the fix
  // is a per-node cache keyed on node.id + the data fields resolveHandles reads).
  ...
  // empty ones as "+ Add step" placeholders — before any child edge exists.
  // Which source handle is the continuation (and so NOT a lane) is decided by
  // resolveBranchHandleIds; see its doc for the precedence and for why only an
  // explicitly flagged registry default is trusted. The registry lookup mirrors
  // the other `getDefaultHandle` call sites in this file (findResolvedHandle,
  // onLaneAdd, tailSlot) so all four agree on what a node's spine output is.
  const getBranchHandles = useCallback(
    (node: N) => {
      ...
      const defaultHandle = node.type ? registry?.getDefaultHandle(node.type, 'source') : undefined;
      const laneIds = resolveBranchHandleIds(
        candidates,
        defaultHandle?.isDefaultForType ? defaultHandle.id : undefined
      );
      return laneIds.map((id) => ({ id, label: resolveBranchLabel(node.id, id) }));
    },
  ...
  // Render-phase ref write, and safe to keep as one: it is idempotent -- the value
  // stored is derived purely from THIS render's inputs, never folded over render
  // history -- so a StrictMode double-invoke or a discarded concurrent render
  // writes the identical value. Deliberately not moved into an effect: the tail
  ...
  // this function, and `handleToolbarAction` below branches on the return value,
  // so a `delete` raised in view/readonly mode (or in flow view, or before the
  // projection exists) falls THROUGH to the host's onToolbarAction instead of
  // being silently swallowed by an interception that could not act on it.
  //
  // A host with NEITHER change callback wired is the same case: there is no
  // channel to emit the removal on, so this cannot claim the action. Reporting
  // `true` there would swallow `delete` for a host that handles it through
  // `onToolbarAction` alone (a plausible shape: design mode, host-owned
  // mutations), which is exactly the failure the return value exists to prevent.
  ...
  >(
    (event) => {
      if (event.actionId === 'delete' && deleteStep(event.nodeId)) return;
      onToolbarAction?.(event);
    },
    [deleteStep, onToolbarAction]
  );
