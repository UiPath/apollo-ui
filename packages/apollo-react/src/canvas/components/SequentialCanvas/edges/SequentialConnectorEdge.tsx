...
/**
 * Accessible name for a connector's insert (⊕) button.
 *
 * Every insertable connector renders one of these buttons, and they are real
 * always-focusable `<button>`s, so a screen-reader user tabs through all of them
 * in sequence. Naming them all "Insert step" makes the whole set
 * indistinguishable; each one is therefore named by the step it acts relative to,
 * using the same stable pre-order numbers the gutter prints (D7), so the name
 * survives collapsing a subtree.
 *
 * The bare fallback is retained for the one case with no numbered row on either
 * end (a degraded projection where neither endpoint carries a step number);
 * it is a worse name than the others, not a wrong one.
 *
 * Exported for direct unit testing, since asserting on it otherwise means
 * mounting an edge inside a live ReactFlow store.
 */
export function resolveInsertStepLabel(
  translate: ReturnType<typeof useSafeLingui>['_'],
  anchor: SequentialInsertAnchor | undefined
): string {
  if (anchor?.kind === 'branch') {
    return translate({
      id: 'sequential-canvas.insert-step.branch',
      message: 'Insert step into {branchLabel} of step {stepNumber}',
      values: { branchLabel: anchor.branchLabel, stepNumber: anchor.stepNumber },
    });
  }
  if (anchor?.kind === 'before') {
    return translate({
      id: 'sequential-canvas.insert-step.before',
      message: 'Insert step before step {stepNumber}',
      values: { stepNumber: anchor.stepNumber },
    });
  }
  if (anchor?.kind === 'after') {
    return translate({
      id: 'sequential-canvas.insert-step.after',
      message: 'Insert step after step {stepNumber}',
      values: { stepNumber: anchor.stepNumber },
    });
  }
  return translate({ id: 'sequential-canvas.insert-step', message: 'Insert step' });
}
...
export const SequentialConnectorEdge = memo(function SequentialConnectorEdge({
  ...
  const geometry = useEdgeGeometry({
    ...
    // `enableLineJumps` is deliberately left off. The projection routes every
    // connector on a deterministic orthogonal rail, so crossings are structural
    // (a branch lane passing its owner's trunk) rather than incidental, and
    // hopping them would read as noise on top of an already-explicit layout.
  });
    ...
    <>
      <g onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
        ...
        {hasLabel && kind !== 'branch-entry' && (
          <EdgeLabel x={labelPoint.x} y={labelPoint.y} text={label as string} borderColor={color} />
        )}
      </g>
