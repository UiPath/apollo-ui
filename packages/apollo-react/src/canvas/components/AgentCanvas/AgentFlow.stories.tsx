 ...
 * Demonstrates the ability to drag the agent node in design mode
 */
const DesignModeWithDraggingComponent = (_args: AgentFlowProps) => {
  ...
  // Create sample resources with one having a custom position
  const resourcesWithPosition: AgentFlowResource[] = sampleResources.map((resource, index) => {
    // Add position to the first resource (tool-slack)
    if (index === 0) {
      return {
        ...resource,
        position: { x: 1400, y: 700 },
      } as AgentFlowResource & { position: { x: number; y: number } };
    }
    return resource;
  });

  const [resources, setResources] = useState<AgentFlowResource[]>(resourcesWithPosition);
  const [selectedResourceId, setSelectedResourceId] = useState<string | null>(null);
  ...
  const handleSetResourceNodePosition = useCallback((resourceId: string, position: { x: number; y: number }) => {
    setResources((prev) =>
      prev.map((r) => {
        // Match resource by ID (extract from the compound node ID)
        if (resourceId.endsWith(`:${r.id}`)) {
          return {
            ...r,
            position,
          } as AgentFlowResource & { position: { x: number; y: number } };
        ...
        >
          ...
          <div style={{ fontSize: "0.75rem", fontFamily: "monospace", maxHeight: "400px", overflowY: "auto" }}>
            {resources.map((resource) => {
              const resourceWithPos = resource as AgentFlowResource & { position?: { x: number; y: number } };
              return (
                <div
                  key={resource.id}
                  style={{
                ...
                >
                  ...
                  {resourceWithPos.position ? (
                    <div style={{ color: "#2563eb" }}>
                      <div>
                        <strong>X:</strong> {resourceWithPos.position.x.toFixed(2)}
                      </div>
                      <div>
                        <strong>Y:</strong> {resourceWithPos.position.y.toFixed(2)}
                      </div>
                    </div>
                  ) : (
