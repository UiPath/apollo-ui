...
const StageNodeComponent = (props: StageNodeProps) => {
  ...
  const handleToolboxItemSelected = useCallback(
    (item: ListItem) => {
      if (toolboxMode === 'replace' && replaceTaskRef.current && onTaskReplace) {
        onTaskReplace(item, replaceTaskRef.current.groupIndex, replaceTaskRef.current.taskIndex);
    ...
    [toolboxMode, onTaskReplace, onAddTaskFromToolbox, onReplaceTaskTargetChange, setSelectedNodeId, id]
  );
