...
vi.mock("@xyflow/react", () => ({
  ...
  Handle: ({ type, position, id, ...props }: any) => <div data-testid={`handle-${type}-${position}-${id}`} {...props} />,
  BaseEdge: ({ id, path, style, markerEnd }: any) => <path data-testid={`edge-${id}`} d={path} style={style} markerEnd={markerEnd} />,
...
graph LR
  ...
  beforeEach(async () => {
    ...
    console.error = (...args: any[]) => {
      ...
      if (
        message.includes("Warning: An update to CodedAgentFlowInner inside a test was not wrapped in act") ||
