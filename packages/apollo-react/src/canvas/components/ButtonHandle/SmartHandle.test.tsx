...
vi.mock("@xyflow/react", () => ({
  Handle: ({ children, ...props }: any) => {
    ...
    return (
      <div data-testid="smart-handle" data-position={domProps.position} data-type={domProps.type} data-id={domProps.id} {...domProps}>
        {children}
      </div>
    );
  },
...
describe("SmartHandle", () => {
      ...
      mockUseStore.mockImplementation((selector: any) => {
        const state = {
          nodes: [
            ...
            { id: "target-node", position: { x: 400, y: 100 }, measured: { width: 100, height: 50 } },
          ],
          edges: [{ id: "e1", source: "test-node", sourceHandle: "output", target: "target-node", targetHandle: "input" }],
        };
        return selector(state);
      });
      ...
      mockUseStore.mockImplementation((selector: any) => {
        const state = {
          nodes: [
            ...
            { id: "target-node", position: { x: 100, y: 100 }, measured: { width: 100, height: 50 } },
          ],
          edges: [{ id: "e1", source: "test-node", sourceHandle: "output", target: "target-node", targetHandle: "input" }],
        };
        return selector(state);
    ...
    it("positions target handle toward connected source node", () => {
      mockUseStore.mockImplementation((selector: any) => {
        const state = {
          nodes: [
            { id: "test-node", position: { x: 400, y: 100 }, measured: { width: 100, height: 50 } },
            { id: "source-node", position: { x: 100, y: 100 }, measured: { width: 100, height: 50 } },
          ],
          edges: [{ id: "e1", source: "source-node", sourceHandle: "output", target: "test-node", targetHandle: "input" }],
        };
        return selector(state);
      });
    ...
    it("positions handle based on vertical relationship (bottom)", () => {
      mockUseStore.mockImplementation((selector: any) => {
        const state = {
          nodes: [
            { id: "test-node", position: { x: 100, y: 100 }, measured: { width: 100, height: 50 } },
            { id: "target-node", position: { x: 100, y: 300 }, measured: { width: 100, height: 50 } },
          ],
          edges: [{ id: "e1", source: "test-node", sourceHandle: "output", target: "target-node", targetHandle: "input" }],
        };
        return selector(state);
    ...
    it("allows target handles to use right side when no source handles exist on the node", () => {
      mockUseStore.mockImplementation((selector: any) => {
        const state = {
          nodes: [
            { id: "test-node", position: { x: 100, y: 100 }, measured: { width: 100, height: 50 } },
            { id: "source-node", position: { x: 400, y: 100 }, measured: { width: 100, height: 50 } },
          ],
          edges: [{ id: "e1", source: "source-node", sourceHandle: "output", target: "test-node", targetHandle: "input" }],
        };
        return selector(state);
  ...
  describe("Edge cases", () => {
    ...
    it("handles node not found in store", () => {
      mockUseStore.mockImplementation((selector: any) => {
        const state = {
          nodes: [{ id: "other-node", position: { x: 100, y: 100 }, measured: { width: 100, height: 50 } }],
          edges: [],
        };
        return selector(state);
      });
    ...
    it("handles no edges (unconnected)", () => {
      mockUseStore.mockImplementation((selector: any) => {
        const state = {
          nodes: [{ id: "test-node", position: { x: 100, y: 100 }, measured: { width: 100, height: 50 } }],
          edges: [],
        };
        return selector(state);
      });
    ...
    it("handles connected node not found", () => {
      mockUseStore.mockImplementation((selector: any) => {
        const state = {
          nodes: [{ id: "test-node", position: { x: 100, y: 100 }, measured: { width: 100, height: 50 } }],
          edges: [{ id: "e1", source: "test-node", sourceHandle: "output", target: "missing-node", targetHandle: "input" }],
        };
        return selector(state);
      });
    ...
    it("handles multiple connections (uses average position)", () => {
      mockUseStore.mockImplementation((selector: any) => {
        const state = {
          ...
          edges: [
            { id: "e1", source: "test-node", sourceHandle: "output", target: "target-1", targetHandle: "input" },
            { id: "e2", source: "test-node", sourceHandle: "output", target: "target-2", targetHandle: "input" },
          ],
        };
        return selector(state);
      });
