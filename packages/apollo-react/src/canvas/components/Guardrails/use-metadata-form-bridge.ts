...
export interface MetadataFormBridgeOptions {
  ...
  /** Whether the resolver reports as the user edits. See `GuardrailValidatorFormProps`. */
...
export function useMetadataFormBridge({
  ...
  // Revalidate the moment the host opens the gate, so the messages appear on the Save attempt
  // that revealed them rather than on the next keystroke.
  //
  // `trigger()` rather than relying on the `mode` switch alone: react-hook-form does re-read
  // `_options.mode` after mount (7.66's `useForm` reassigns it in an effect and reads it lazily
  // per event), but that is an implementation detail rather than a documented contract, and a
  // lockfile bump could take it away silently. This makes the first paint independent of it.
  const wasValidatingLiveRef = useRef(validateLive);
  useEffect(() => {
    const openedGate = validateLive && !wasValidatingLiveRef.current;
    wasValidatingLiveRef.current = validateLive;
    if (!openedGate) return;
    void formRef.current?.trigger();
  }, [validateLive]);

  // Range constraints are only worth declaring if something evaluates them: this form has no
  // submit of its own, so under 'onSubmit' the resolver never runs at all. That silence is the
  // point before the host opens the gate — the builder's own checks still block Save meanwhile,
  // and the effect above fills the messages in on the failed attempt.
  return { plugins, schemaMode: validateLive ? 'onChange' : 'onSubmit' };
}
