...
describe('getCentralizedGuardrailItemId', () => {
  ...
  it('keeps fields apart that a joined string would run together', () => {
    expect(
      getCentralizedGuardrailItemId(guardrail({ validator: 'a-b', executionStage: 'c' }))
    ).not.toBe(getCentralizedGuardrailItemId(guardrail({ validator: 'a', executionStage: 'b-c' })));
    expect(getCentralizedGuardrailItemId(guardrail({ isByo: true, name: 'builtin' }))).not.toBe(
      getCentralizedGuardrailItemId(guardrail())
    );
  });
...
describe('getCentralizedGuardrailDisplay', () => {
  ...
  it('never gives a BYO guardrail the curated name, even when it has none of its own', () => {
    expect(
      getCentralizedGuardrailDisplay({ validator: 'pii_detection', isByo: true }, { copy }).name
    ).toBe('pii_detection');
  });
...
describe('resolveCentralizedGuardrailParameters, BYO guardrails', () => {
  ...
  it('renders a scalar as a value even when `parameterType` claims a map', () => {
    expect(
      resolveCentralizedGuardrailParameters(
        byo([{ id: 'limit', parameterType: 'map-enum', value: 3 }]),
        { labels: FALLBACK_LABELS }
      )
    ).toEqual([{ id: 'limit', kind: 'value', label: 'limit', value: '3' }]);
  });
  ...
  it('keeps the key list as its own row when the threshold map was persisted empty', () => {
    const rows = resolveCentralizedGuardrailParameters(
      byo([
        { id: 'entities', value: ['Email'] },
        { id: 'entityThresholds', value: null },
      ]),
      {
        definition: {
          validator: 'pii_detection',
  ...
  it('reads option labels and thresholds as own properties only', () => {
    const rows = resolveCentralizedGuardrailParameters(
      byo([
        { id: 'mode', value: 'constructor' },
        { id: 'entities', value: ['toString'] },
        { id: 'entityThresholds', value: JSON.parse('{"__proto__": 0.5}') },
      ]),
      {
        definition: {
          validator: 'pii_detection',
          byoValidatorName: 'Acme PII',
          parameters: [
            { id: 'mode', type: 'enum', label: 'Mode', optionLabels: {} },
    ...
    expect(rows).toEqual([
      { id: 'mode', kind: 'value', label: 'Mode', value: 'constructor' },
      {
        id: 'entityThresholds',
        kind: 'thresholds',
        label: 'Detection thresholds',
        thresholds: [
          { key: '__proto__', label: '__proto__', value: 0.5 },
          { key: 'toString', label: 'toString', value: undefined },
        ],
      },
    ]);
  });
