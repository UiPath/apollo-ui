...
describe('buildGuardrailFormSchema', () => {
  ...
  it('splits enum-list at the inline-chip cap: chips custom component vs multiselect', () => {
    ...
    expect(fieldFor(small)).toMatchObject({
      ...
      // Without a valueType a custom field validates as `z.any()`, where `required` silently
      // does nothing — the chip path would then be the only enum-list that never shows a live
      // required error, while the multiselect path above does.
  ...
  it('declares a valueType for host-overridden fields, matching the parameter type', () => {
    const overridden = (type: GuardrailParameterDefinition['type']) =>
      buildGuardrailFormSchema(
        [{ id: 'p', type, label: 'P', required: true, defaultValue: null } as never],
        LABELS,
        { overriddenIds: new Set(['p']) }
      ).sections[0].fields[0];

    expect(overridden('text')).toMatchObject({ type: 'custom', valueType: 'string' });
    expect(overridden('number')).toMatchObject({ type: 'custom', valueType: 'number' });
    expect(overridden('boolean')).toMatchObject({ type: 'custom', valueType: 'boolean' });
    expect(overridden('enum-list')).toMatchObject({ type: 'custom', valueType: 'string-array' });
    // map-enum's Record<string, number> has no CustomValueType counterpart, so its required
    // check stays the host's (`getRequiredEmptyParameterIds` treats an empty map as empty).
    expect(overridden('map-enum')).toMatchObject({ type: 'custom', valueType: undefined });
  });
