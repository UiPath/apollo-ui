...
describe('GuardrailList', () => {
  ...
  describe('reorder', () => {
    ...
    it('keeps the handle inside the row it belongs to', () => {
      renderList({ onReorder: vi.fn() });

      // The row paints a hover tint with 4px of padding, which is exactly the reach of the
      // handle's `ring-offset-2` focus ring, so a negative margin here puts the handle's hover
      // surface and its ring outside the tinted box. Pinned as a class because happy-dom has
      // no layout to measure.
      expect(reorderHandle('PII detection 1').className).not.toMatch(/-m[lxs]?-/);
    });
  ...
  it('hands byoChip to the rows, so only the BYO one is badged', () => {
    renderList({ byoChip: true });

    // GUARDRAILS is one UiPath validator, one BYO, one custom: exactly one chip.
    expect(screen.getAllByText('BYO')).toHaveLength(1);
    expect(
      screen.getByText('Noma prompt shield').closest('[data-slot="guardrail-list-row"]')
    ).toHaveTextContent('BYO');
  });
