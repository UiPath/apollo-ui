...
> {
  /**
   * The guardrails this policy enforces on the agent being edited, already filtered for the
   * agent kind. `getApplicableCentralizedGuardrails` is the predicate both products run.
   */
   ...
   * parameter labels. Leave `undefined` while the catalog is loading: that is what keeps a
   * row from claiming a configuration was deleted before anything has been fetched.
   */
 ...
 * enforces on an agent. They are not editable anywhere in the product, so this section shows
 * what is enforced and hands a selection back; the details view is a sibling export.
 *
 * Governance guardrails are their own record, not a variant of the locally configured ones:
 * see `CentralizedGuardrail`. Nothing here reads a context — both products hold the policy in
 * one of their own, and passing it in is what lets the same component serve both.
 */
...
>({
  ...
  if (guardrails.length === 0) {
    // An explicit `null` is honoured: both products render nothing at all here, so that is
    // the default rather than a built-in empty line.
