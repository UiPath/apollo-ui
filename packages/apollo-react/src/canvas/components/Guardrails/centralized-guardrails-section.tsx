...
> {
  /** Already filtered by `getApplicableCentralizedGuardrails`. */
  guardrails: CentralizedGuardrail[];
  /** Name of the AI Trust Layer policy enforcing them; shown in the caption. */
  policyName: string;
  /** For provider names and configuration status. `undefined` while the catalog loads. */
  definitions?: TDefinition[];
  /** The host opens the details (a dialog, a panel overlay). */
  onSelect?: (guardrail: CentralizedGuardrail) => void;
  /** Documentation link inside the info popover. */
  docsHref?: string;
  /** Replace the localized scope names. Defaults to the family's own scope labels. */
  formatScope?: (scope: GuardrailScope) => string;
  /** Rendered when there are no guardrails. Defaults to nothing. */
...
/** The read-only guardrails an AI Trust Layer policy enforces on an agent. */
