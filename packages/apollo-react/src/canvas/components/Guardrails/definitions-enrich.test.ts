...
describe('enrichGuardrailDefinitions', () => {
  ...
  it.each([
    ...
    '__proto__',
    'hasOwnProperty',
  ])('treats %s as an uncurated validator rather than a copy table entry', (validator) => {
    // The copy table is a plain object, so a bare lookup on these returns a prototype member
    // it never declared. Enrichment would then read `paramLabels` off a function and throw,
    // losing every definition in the payload over one the table simply has no copy for.
    const definition = enrichOne({
      ...PII_DETECTION_WIRE,
      validator,
      displayName: 'Backend name',
    ...
    expect(definition.displayName).toBe('Backend name');
    expect(definition.description).toBe('Backend description');
    expect(definition.usageNote).toBeUndefined();
    // Falls back through the wire label, then the humanized id, exactly as any other
    // validator the curated table does not cover.
    expect(param(definition, 'entityThresholds').label).toBe('Entity thresholds');
  });
});
...
describe('humanizeGuardrailParameterId', () => {
  ...
  // Splitting on every capital used to produce "U r l allow list" and "F i passport number",
  // which is worse than the raw id this fallback exists to improve on - so it broke on exactly
  // the ids that have no curated label. Both fail against that implementation.
  it.each([
    ['URLAllowList', 'URL allow list'],
    ['FIPassportNumber', 'FI passport number'],
    ['ABC', 'ABC'],
  ])('keeps the acronym run in %s whole', (id, expected) => {
    expect(humanizeGuardrailParameterId(id)).toBe(expected);
  });

  // No token to build a label from: returning the id beats returning an empty string, which
  // would render as a nameless field.
  it.each([[''], ['__'], ['---']])('falls back to the id itself for %j', (id) => {
    expect(humanizeGuardrailParameterId(id)).toBe(id);
  });
});
