...
import { Button, FormField, FormFieldLabel, TooltipProvider } from '@uipath/apollo-wind';
...
Values are fully controlled by the host (\`parameters\` + \`onChange\`). Validation is shared:
the schema declares \`required\`/\`min\`/\`max\` from the definitions and MetadataForm's resolver
evaluates them as the user types with localized messages, so a field can show an error with no
\`errors\` entry. The host owns domain validation and the save-time gate (\`errors\` +
\`onClearError\`): compute those with the exported \`getRequiredEmptyParameterIds\` and
\`getOutOfRangeParameterIds\` helpers, which are authoritative where the two disagree. Individual
...
The component's own chrome strings localize through lingui (\`useSafeLingui\`, English
fallback without a provider; mount \`ApI18nProvider component="canvas"\` for translations),
overridable per string via \`labels\`.
        `,
      },
    },
  },
...
/**
 * The host-owned half of validation: this story computes the error map with
 * getRequiredEmptyParameterIds on save, exactly as a product form would, and those messages
 * win over the resolver's. The form also runs its own required/range rules as you type, so
 * errors can appear here before any save.
 */
...
/** Component chrome strings resolved from the canvas lingui catalog (Japanese). */
export const Localized: Story = {
  ...
  render: (args) => (
    <ApI18nProvider component="canvas" locale="ja">
      <ControlledForm parameterDefinitions={args.parameterDefinitions} />
    </ApI18nProvider>
  ),
};
