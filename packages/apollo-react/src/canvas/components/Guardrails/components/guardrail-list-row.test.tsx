...
describe('GuardrailListRow', () => {
  ...
  describe('chips', () => {
    ...
    it('renders the preview badge only for built-in validators, and only when asked', () => {
      ...
      // Blue, which is the colour both products give this chip today. The label lives in an
      // inner span so it can truncate, so the tone is on its parent.
      expect(screen.getByText('Preview').closest('[data-slot="guardrail-status-chip"]')).toHaveClass(
        'bg-info-background'
      );
      unmount();
    ...
    it('renders the BYO badge only for BYO rows, and only when asked', () => {
      const { unmount } = renderRow(BYO_GUARDRAIL, { byoChip: true });
      // Green, which is the colour both products give this chip today.
      expect(screen.getByText('BYO').closest('[data-slot="guardrail-status-chip"]')).toHaveClass(
        'bg-success-background'
      );
      unmount();

      // A UiPath validator is not BYO, and neither is a custom guardrail.
      const uipath = renderRow(PII_GUARDRAIL, { byoChip: true });
      expect(screen.queryByText('BYO')).not.toBeInTheDocument();
      uipath.unmount();

      // Opt-in: the row stays bare for a host that does not ask for it.
      renderRow(BYO_GUARDRAIL, {});
      expect(screen.queryByText('BYO')).not.toBeInTheDocument();
    });

    it('puts provenance before lifecycle, since a BYO row is also a built-in validator', () => {
      renderRow(BYO_GUARDRAIL, { byoChip: true, previewChip: true });

      const chips = screen
        .getAllByText(/^(BYO|Preview)$/)
        .map((label) => label.textContent?.trim());
      expect(chips).toEqual(['BYO', 'Preview']);
    });
  });
