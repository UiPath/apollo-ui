...
export function MapEnumField(props: CustomFieldComponentProps) {
  ...
  // No selected keys means nothing to configure, so the field normally renders nothing. But
  // the builder still gates Save on a required map's host error, and returning null there left
  // the user blocked with no label and no message anywhere on screen. With an error present,
  // render the label and the message even though there are no rows.
  if (keys.length === 0) {
