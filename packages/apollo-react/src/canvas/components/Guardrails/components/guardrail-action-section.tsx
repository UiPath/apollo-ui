...
export interface GuardrailActionSectionProps {
  ...
  /** Called with the whole next action; switching the type resets the payload. */
 ...
 *
 * Rendered inside `GuardrailBuilder`, and exported for hosts composing their own builder
 * (the custom-rules path). Standalone it needs `action` and `onActionChange` only: labels
 * come from the catalog, and every slot falls back.
 */
