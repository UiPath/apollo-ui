...
//
// One map for every label set: `policyName` is the centralized section's, and passing it to
// the form and builder strings, which never declare that placeholder, is inert.
 ...
 * details. Domain strings stay elsewhere: a validator's display name and description come
 * from the canonical copy table (`definitions-copy.ts`) and a BYO configuration's from the
 * connector's own definition.
 */
...
//
// The scope names, the type and description field labels and the action names deliberately
// reuse the builder's ids rather than declaring `guardrails.centralized.*` twins: they name
// the same things with the same English, and a second id would send one string to translators
// twice and let the two drift apart the moment either is revised.
