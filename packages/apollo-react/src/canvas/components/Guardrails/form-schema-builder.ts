...
/**
 * The value shape a `type: 'custom'` field owns, so declared constraints actually bind.
 *
 * Without it apollo-wind validates the field as `z.any()`, where `required` silently does
 * nothing — which would leave the custom editors (chips, map rows, host overrides) without the
 * live required error their first-class equivalents get.
 *
 * `map-enum` has no counterpart in `CustomValueType` (its value is a `Record<string, number>`),
 * so it stays undefined and its required check remains the host's: `getRequiredEmptyParameterIds`
 * treats a map with no keys as empty.
 */
function customValueTypeFor(
  type: GuardrailParameterDefinition['type']
...
function buildGuardrailField(
  ...
  if (options?.overriddenIds?.has(def.id)) {
    return {
      ...
      valueType: customValueTypeFor(def.type),
