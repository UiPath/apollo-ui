...
describe('GuardrailBuilder', () => {
  ...
  describe('required map-enum validation', () => {
    ...
    // The editor keeps rows for deselected keys on purpose, while the save path prunes them
    // through syncMapEnumParameters. Validating the raw values let a required map look filled
    // by a stale key and then persist as {}.
    it('does not call onSave when a required map-enum holds only deselected keys', () => {
      const onSave = vi.fn();
      const definition = makeDef({
        parameters: [
          {
      ...
      render(
        <GuardrailBuilder
          ...
          guardrail={makeGuardrail({
            validatorParameters: [
              // Nothing selected, but a threshold left over from a previous selection.
  ...
  describe('ancestor form safety', () => {
    // Mounted `inline` inside a host's <form>, Enter in one of the builder's own single-line
    // inputs would trigger the host's implicit submission, skipping handleSave's validation
    // and callbacks entirely. The nested MetadataForm guards only the fields it owns.
    // Asserts the mechanism, not the symptom: the test environment does not implement a
    // form's implicit submission, so checking that a host onSubmit went uncalled would pass
    // with or without the guard. Cancelling the keydown is what actually stops the browser
    // from submitting, so that is what is pinned.
  ...
  describe('malformed persisted parameters', () => {
    // The edit path is the only one that receives wire data. A persisted text-list of null hit
    // `param.value.filter(...)` in the guardrailResult memo and threw *during render*, so the
    // builder never mounted at all.
    it('mounts when a persisted text-list arrives as null', () => {
      const definition = makeDef({
        parameters: [
          {
            id: 'positiveExamples',
      ...
      expect(() =>
        render(
          <GuardrailBuilder
            ...
            guardrail={makeGuardrail({
              validatorParameters: [
                { $parameterType: 'text-list', id: 'positiveExamples', value: null } as never,
              ],
            })}
            onSave={vi.fn()}
            onCancel={vi.fn()}
      ...
      expect(screen.getByText('Positive examples')).toBeInTheDocument();
    });

    it('gates Save on a required parameter whose persisted value was malformed', () => {
      const onSave = vi.fn();
      const definition = makeDef({
        parameters: [
          {
  ...
  describe('host sidecar parameters', () => {
    // Overrides write sidecars through onParametersChange; they have no definition here, so the
    // save path must not reach into their shape. Dispatching on the wire discriminator alone
    // did, and a sidecar carrying a malformed text-list crashed the memo during render.
    it('mounts with a malformed sidecar that shares a known discriminator', () => {
      const definition = makeDef({
        parameters: [
          { id: 'guardrailText', type: 'text', label: 'Rule', required: false, defaultValue: '' },
        ],
      });
      ...
      expect(() =>
        render(
          <GuardrailBuilder
            ...
            guardrail={makeGuardrail({
              validatorParameters: [
                { $parameterType: 'text', id: 'guardrailText', value: 'ok' },
                // No definition for this id — a model picker's connection metadata, say.
                { $parameterType: 'text-list', id: 'connectionMeta', value: null } as never,
              ],
            })}
            onSave={vi.fn()}
            onCancel={vi.fn()}
  ...
  describe('multiple instances', () => {
    // The public API supports inline panels, so two builders can share a document. Any constant
    // control id duplicates, and `htmlFor` then binds a label to whichever instance rendered
    // first.
    //
    // The assertion is deliberately "nothing in the container repeats", with a single explicit
    // exclusion, rather than an allow-list of id shapes. The previous version filtered on a
    // pattern and so silently stepped over `severity-level`, `block-reason` and
    // `ootb-enable-evals` — the very ids it was meant to catch. An exclusion list fails loudly
    // when a new control is added; an allow-list quietly stops covering it.
    it('renders no duplicate ids across two inline builders', () => {
      // Real parameters, so MetadataForm actually mounts — the previous test used an empty
      // definition and never exercised it at all.
      const definition = makeDef({
        parameters: [
          {
            id: 'threshold',
      ...
      // MetadataForm derives a field's DOM id from its schema field name (`def.id`), so the
      // parameter editors legitimately repeat across instances. That is apollo-wind's id scheme
      // and cannot be namespaced here without breaking the name-to-value mapping.
      const metadataFormOwned = new Set(definition.parameters.map((param) => param.id));

      const ids = [...container.querySelectorAll('[id]')]
        .map((el) => el.id)
        .filter((id) => !metadataFormOwned.has(id));

      const duplicates = ids.filter((id, i) => ids.indexOf(id) !== i);
      expect(duplicates).toEqual([]);

      // And every label resolves to exactly one control.
      for (const label of container.querySelectorAll('label[for]')) {
        const target = label.getAttribute('for');
        if (target && !metadataFormOwned.has(target)) {
...
describe('saveDisabled passthrough', () => {
  ...
  // QA finding #3: the dialog ran two validation engines on two timings. The MetadataForm
  // resolver reported the moment a field was edited, while the builder's own checks — name,
  // scopes, action, and its parameter checks — waited for a Save attempt. Clearing a
  // multi-select went red on the spot; clearing every scope said nothing.
  //
  // These assert the RESOLVER's message ('Must be at most 1'), not the builder's own
  // ('Value is out of range'). Only the resolver's timing changed, and the builder's message
  // was already gated, so asserting on that one passes with or without the fix.
  describe('parameter errors wait for the first Save attempt', () => {
    const RESOLVER_MESSAGE = 'Must be at most 1';
    ...
    it('stays silent while the value is edited out of range', async () => {
      renderBuilder();
      ...
      await expect(screen.findByText(RESOLVER_MESSAGE)).rejects.toThrow();
      expect(screen.queryByText('Value is out of range')).not.toBeInTheDocument();
    });

    it('reports on the failed Save that revealed it', () => {
    ...
    // The gate only ever opens, so once Save has failed the field has to keep up with edits
    // rather than freezing on the message from that attempt.
    it('validates live once the gate has opened', async () => {
      renderBuilder();
      ...
      expect(screen.queryByText(RESOLVER_MESSAGE)).not.toBeInTheDocument();
    });
  });
});
