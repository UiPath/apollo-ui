...
export function RenderParameterBridge(props: CustomFieldComponentProps) {
  ...
  const node = ctx.renderParameter({
    definition: def,
    value: entry?.value,
    // Host error wins, then the resolver's. Forwarding only the host's meant an override
    // never saw the live required/range error the typed custom-field validation produces,
    // so an overridden parameter was the one editor that stayed silent when `errors` was
    // omitted — contradicting the contract the surrounding form advertises.
    error: ctx.errors?.[props.name] ?? props.error,
