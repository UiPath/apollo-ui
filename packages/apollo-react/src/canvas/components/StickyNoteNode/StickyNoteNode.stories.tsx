...
function createBaseNode(
  ...
  handleConfigurations: Array<{
    position: Position;
    handles: Array<{ id: string; type: "source" | "target"; handleType: "input" | "output"; label: string }>;
  }>
): Node {
  return {
    id,
...
function createPipelineNodes(): Node[] {
  return [
    ...
    createBaseNode("node-source", "cloud-upload", "Data Source", "Input Stream", { x: 128, y: 224 }, [
      { position: Position.Right, handles: [{ id: "output", type: "source", handleType: "output", label: "Output" }] },
    ]),
    createBaseNode("node-filter", "filter", "Filter", "Validation Rules", { x: 416, y: 224 }, [
      { position: Position.Left, handles: [{ id: "input", type: "target", handleType: "input", label: "Input" }] },
      {
    ...
    createBaseNode("node-transform", "settings", "Transform", "Data Processing", { x: 800, y: 144 }, [
      { position: Position.Left, handles: [{ id: "input", type: "target", handleType: "input", label: "Input" }] },
      { position: Position.Right, handles: [{ id: "output", type: "source", handleType: "output", label: "Output" }] },
    ]),
    ...
    createBaseNode("node-merge", "git-merge", "Merge", "Combine Streams", { x: 1216, y: 224 }, [
      ...
      { position: Position.Right, handles: [{ id: "output", type: "source", handleType: "output", label: "Merged" }] },
    ]),
    ...
    createBaseNode("node-storage", "database", "Storage", "Database", { x: 1664, y: 144 }, [
      { position: Position.Left, handles: [{ id: "input", type: "target", handleType: "input", label: "Input" }] },
      { position: Position.Right, handles: [{ id: "output", type: "source", handleType: "output", label: "Logs" }] },
    ]),
    createBaseNode("node-index", "search", "Index", "Search Engine", { x: 1664, y: 304 }, [
      { position: Position.Left, handles: [{ id: "input", type: "target", handleType: "input", label: "Input" }] },
      { position: Position.Right, handles: [{ id: "output", type: "source", handleType: "output", label: "Events" }] },
    ]),
...
function createPipelineEdges(): Edge[] {
  return [
    ...
    { id: "e-merge-index", source: "node-merge", sourceHandle: "output", target: "node-index", targetHandle: "input", type: "smoothstep" },
    // Analytics flows
    {
...
function WithBaseNodesStory() {
  ...
  const { canvasProps } = useCanvasStory({ initialNodes, initialEdges, additionalNodeTypes: nodeTypes });
