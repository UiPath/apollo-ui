...
export const ApChip = React.forwardRef<HTMLDivElement, ApChipProps>(
  (
    { loading, disabled, tooltip, debugForceOpen = false, error, label, onDelete, ...restProps },
    ref
  ) => {
    ...
    if (tooltip && tooltip.length > 0) {
      return (
        <Tooltip title={tooltip} placement="top" open={debugForceOpen ? true : undefined} arrow>
          <span>{chip}</span>
        </Tooltip>
      );
    }
