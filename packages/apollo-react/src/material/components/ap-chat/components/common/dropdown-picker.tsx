...
export function DropdownPicker<T = string>({
  ...
  const { _ } = useLingui();
  const { spacing, theming, portalContainer } = useChatState();
  ...
  const handleClose = React.useCallback(() => {
    ...
    if (chatServiceInstance) {
      requestAnimationFrame(() => {
        chatServiceInstance.__internalService__.publish(
    ...
    if (useIcon) {
      return (
        <AutopilotChatActionButton
          ...
          ariaLabel={_(
            msg({ id: 'autopilot-chat.common.model-selector', message: `Model selector` })
          )}
        />
      );
    }
    ...
    return (
      ...
      >
        <SelectedOptionContainer
          ...
          aria-expanded={open}
          aria-label={_(
            msg({ id: 'autopilot-chat.common.mode-selector', message: `Mode selector` })
          )}
        >
          {selectedOption.icon && (
  ...
  return (
    <>
      <DropdownPickerContainer>{renderSelectedOption()}</DropdownPickerContainer>
      <Menu
        open={open}
        anchorEl={anchorEl}
        onClose={handleClose}
