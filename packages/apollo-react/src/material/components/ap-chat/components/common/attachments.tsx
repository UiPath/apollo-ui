...
const AttachmentIcon = styled('span')<{
  fileType?: AutopilotChatFileType;
  width?: string;
  height?: string;
...
export const Attachment = React.memo(
  ({ attachment, onRemove, shouldFocus, isFullWidth, loading }: AttachmentProps) => {
    const { _ } = useLingui();
    ...
    const [IconComponent, friendlyType] = React.useMemo(() => {
      const { icon: IconComp, friendlyType: fileType } = fileToIcon(attachment.name);
    ...
    return (
      ...
      >
        <AttachmentIcon fileType={friendlyType}>
          <IconComponent />
        </AttachmentIcon>

        <AutopilotChatTooltip title={attachment.name} placement="top" disableInteractive={true}>
          <ApTypography variant={spacing.primaryFontToken} className="attachment-name">
            {attachment.name}
          </ApTypography>
        ...
        {onRemove && !loading && (
          <div className="attachment-remove">
            <AutopilotChatActionButton
              ...
              iconName="close"
              tooltip={_(
                msg({ id: 'autopilot-chat.common.attachments.remove-file', message: `Remove file` })
              )}
              ariaLabel={_(
                msg({
      ...
      <AttachmentsContainer removeSpacing={removeSpacing} disableOverflow={disableOverflow}>
        {attachments.map((attachment, index) => (
          <Attachment
            ...
            loading={
              attachmentsLoading?.find(
                (a) => a.name === attachment.name && a.size === attachment.size
              )?.loading
            }
