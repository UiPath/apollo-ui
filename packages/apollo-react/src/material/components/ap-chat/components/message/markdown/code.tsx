...
export const Code = React.memo(({ inline, className, children, isStreaming, ...props }: any) => {
  const { _ } = useLingui();
  const { disabledFeatures, spacing, portalContainer } = useChatState();
  ...
  const codeFontStyles = {
    ...
    fontFamily: spacing.compactMode
      ? token.FontFamily.FontMonoSFamily
      : token.FontFamily.FontMonoMFamily,
    lineHeight: spacing.compactMode
      ? token.FontFamily.FontMonoSLineHeight
      : token.FontFamily.FontMonoMLineHeight,
    height: spacing.compactMode
      ? token.FontFamily.FontMonoSLineHeight
      : token.FontFamily.FontMonoMLineHeight,
    fontWeight: spacing.compactMode
      ? token.FontFamily.FontMonoSWeight
      : token.FontFamily.FontMonoMWeight,
  };

  // Additional check for inline code blocks
  const isInline =
    inline || (!className && typeof children === 'string' && !children.includes('\n'));
  ...
  if (match?.[1] === 'math') {
    try {
      ...
      return <pre style={{ color: 'var(--color-foreground)' }}>{String(children)}</pre>;
    }
  }

  const previewContent = (
    <PreviewIframe sandbox="allow-scripts" srcDoc={codeContent} title="HTML Preview" />
  );
  ...
  const markdownContent = language ? (
    <SyntaxHighlighterContainer>
      <SyntaxHighlighter
        ...
        customStyle={{
          margin: 0,
          fontSize: spacing.compactMode
            ? token.FontFamily.FontMonoSSize
            : token.FontFamily.FontMonoMSize,
          fontFamily: spacing.compactMode
            ? token.FontFamily.FontMonoSFamily
            : token.FontFamily.FontMonoMFamily,
    ...
    <NonCodeContentContainer>{children}</NonCodeContentContainer>
  );
      ...
      <CodeBlockContainer>
        <CodeBlockHeader>
          <ApChip
            label={
              language ||
              _(msg({ id: 'autopilot-chat.message.code-block-language', message: `Code` }))
            }
          />
          <div
            style={{
          ...
          >
            {isPreviewable && (
              <AutopilotChatActionButton
                ...
                tooltip={
                  showPreview
                    ? _(msg({ id: 'autopilot-chat.message.code-block-code', message: `Show code` }))
                    : _(
                        msg({ id: 'autopilot-chat.message.code-block-preview', message: `Preview` })
                      )
                }
                onClick={() => {
            ...
            <AutopilotChatActionButton
              ...
              tooltip={_(
                msg({ id: 'autopilot-chat.message.code-block-copy', message: `Copy code` })
              )}
              onClick={() => {
        ...
        {isPreviewable ? (
          <ContentContainer>{showPreview ? previewContent : markdownContent}</ContentContainer>
        ) : (
          markdownContent
        )}
      </CodeBlockContainer>
