...
const getNearestStartMarker = (
  ...
  const candidates = Array.from(
    container.querySelectorAll(`.${CHAT_CITATION_START}[data-citation-ids~="${id}"]`)
  ) as HTMLElement[];
  ...
  for (const marker of candidates) {
    ...
    const isBefore =
      !!(rel & Node.DOCUMENT_POSITION_FOLLOWING) || !!(rel & Node.DOCUMENT_POSITION_CONTAINS);
    if (isBefore) {
      nearest = marker;
    }
  }
    ...
    return (
      ...
      >
        <Box
          ...
          aria-label={_(
            msg({
              id: 'autopilot-chat.message.citation-aria-label',
              message: `Citation ${id}: ${title}${pageText}`,
            })
          )}
          onClick={handleClick}
