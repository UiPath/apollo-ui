...
const MainContainer = styled('div')<{ historyOpen: boolean }>(
  ({ historyOpen }: { historyOpen: boolean }) => ({
    ...
    ...(historyOpen && {
      width: `calc(100% - ${CHAT_DRAWER_WIDTH_FULL_SCREEN}px - 2 * ${token.Spacing.SpacingBase})`,
    }), // account for padding
  })
);
  ...
  return (
    <>
      ...
      <ContentContainer ref={setFullScreenContainer}>
        ...
        {!settingsDisabled && <AutopilotChatSettings open={settingsOpen} isFullScreen={true} />}
      </ContentContainer>
    </>
  );
};
