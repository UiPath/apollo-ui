...
export const useAudioOutput = ({
...
}: AudioOutputHookProps = {}): AudioOutputHookResult => {
  ...
  const initWorker = useCallback(() => {
    if (!workerRef.current) {
      ...
      workerRef.current.onmessage = (event: MessageEvent) => {
        const { id, floatData, error } = event.data;
  ...
  const decodeAudioData = useCallback(
    async (base64Data: string): Promise<AudioBuffer> => {
  ...
  const queueOutputAudio = useCallback(
    (mimeType: string, base64Data: string, sequenceNumber: number) => {
      ...
      if (sequenceNumber != 0 && sequenceNumber != sequenceNumberRef.current) {
        console.warn(
          `[AudioPlayer] sequence number ${sequenceNumber} received when ${sequenceNumberRef.current} was expected.`
        );
      }
