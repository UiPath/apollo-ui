...
export function calculateDynamicPadding(currentWidth: number): number {
  ...
  const percentage =
    (currentWidth - CHAT_WIDTH_SIDE_BY_SIDE_MIN) /
    (CHAT_WIDTH_SIDE_BY_SIDE_MAX - CHAT_WIDTH_SIDE_BY_SIDE_MIN);

  // Calculate the padding based on the percentage and round to integer
  return Math.round(
    CHAT_MESSAGE_MIN_PADDING + (CHAT_MESSAGE_MAX_PADDING - CHAT_MESSAGE_MIN_PADDING) * percentage
  );
}
