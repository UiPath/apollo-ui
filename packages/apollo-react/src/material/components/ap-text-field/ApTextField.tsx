 ...
 */
export function ApTextField<T extends InputType = 'text'>(props: Readonly<ApTextFieldProps<T>>) {
  const {
    label,
    value,
    size = 'tall',
  ...
  const cachedProps = useMemo<Partial<TextFieldProps>>(() => {
    ...
    if (errorMessage) {
      ...
      helperTextValue = (
        <ApTypography variant={FontVariantToken.fontSizeS}>{helperText}</ApTypography>
      );
    }
  ...
  }, [readOnly, size, helperText, error, errorMessage, startAdornment, endAdornment, customStyle]);
  ...
  const shouldShowTooltip = (readOnly && !!readOnlyTooltip) || (isOverflowing && !!value);
