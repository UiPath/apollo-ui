// Auto-generated from studio-activities-icon-sets/activities-ui-path/ui-path-ui-path.svg
import React from 'react';

export interface UIPathUIPathProps extends Omit<React.SVGProps<SVGSVGElement>, 'width' | 'height'> {
  /**
   * Size to apply to both width and height.
   * @default 24
   */
  size?: string | number;
}

export const UIPathUIPath = React.forwardRef<SVGSVGElement, UIPathUIPathProps>(
  ({ size, ...props }, ref) => (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      ref={ref}
      {...props}
      width={size ?? 24}
      height={size ?? 24}
    >
      <g clipPath="url(#clip0_4117_2243)">
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M15.6179 17.3024H17.6084V9.47711H15.6179V17.3024ZM17.8341 7.34C17.8341 8.04697 17.3202 8.52899 16.6141 8.52899C15.9079 8.52899 15.394 8.04697 15.394 7.34C15.394 6.61697 15.9079 6.11888 16.6141 6.11888C17.3202 6.11888 17.8341 6.61697 17.8341 7.34ZM12.0546 7.34001H14.0452V13.1729C14.0452 15.808 12.5522 17.3989 10.032 17.3989C7.5599 17.3989 6.09905 15.8401 6.09905 13.1729V7.34001H8.0896V13.1729C8.0896 14.6351 8.69955 15.5188 10.0801 15.5188C11.4126 15.5188 12.0546 14.6833 12.0546 13.1729V7.34001ZM4.36597 19.6323H19.6158V4.36735H4.36597V19.6323ZM2.375 21.625H21.606V2.375H2.375V21.625Z"
          fill="#FA4616"
        />
      </g>
      <defs>
        <clipPath id="clip0_4117_2243">
          <rect width="24" height="24" fill="var(--color-foreground)" />
        </clipPath>
      </defs>
    </svg>
  )
);

UIPathUIPath.displayName = 'UIPathUIPath';

export default UIPathUIPath;
