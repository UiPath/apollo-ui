// Auto-generated from ui-agents-icons/agent-trace/agent-trace-conversation-citation.svg
import React from 'react';

export interface AgentTraceConversationCitationProps
  extends Omit<React.SVGProps<SVGSVGElement>, 'width' | 'height'> {
  /**
   * Size to apply to both width and height.
   * @default 24
   */
  size?: string | number;
}

export const AgentTraceConversationCitation = React.forwardRef<
  SVGSVGElement,
  AgentTraceConversationCitationProps
>(({ size, ...props }, ref) => (
  <svg
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    ref={ref}
    {...props}
    width={size ?? 24}
    height={size ?? 24}
  >
    <path
      d="M19.2979 19.383H20.0099V14.151H19.2979V13.159H21.2499V20.375H19.2979V19.383Z"
      fill="#0067DF"
    />
    <path
      d="M17.8956 19.8269L16.5389 19.8268V17.0028C16.5389 16.9495 16.5389 16.8535 16.5389 16.7148C16.5389 16.5762 16.5415 16.4215 16.5469 16.2508C16.5522 16.0802 16.5575 15.9282 16.5629 15.7948C16.4935 15.8748 16.4295 15.9442 16.3709 16.0028C16.3175 16.0615 16.2615 16.1148 16.2029 16.1628L15.5549 16.6988L14.7549 15.7148L16.7149 14.1148L17.8956 14.1149V19.8269Z"
      fill="#0067DF"
    />
    <path
      d="M14.2411 20.375H12.2891V13.159H14.2411V14.151H13.5291V19.383H14.2411V20.375Z"
      fill="#0067DF"
    />
    <path
      d="M19.1729 3.65002C19.762 3.65002 20.2558 3.84436 20.6533 4.23303C21.0507 4.62171 21.2499 5.09778 21.25 5.66077V11.668H19.625V5.65979C19.625 5.56356 19.5784 5.47513 19.4863 5.39514C19.3941 5.31506 19.2917 5.27502 19.1797 5.27502H5.59082L7.14355 6.87561V15.5419C7.14355 15.6382 7.18993 15.7266 7.28223 15.8066C7.37428 15.8867 7.47663 15.9267 7.58887 15.9267H10.8161V17.5517H7.60254C7.00859 17.5517 6.51273 17.3572 6.11523 16.9687C5.71794 16.5801 5.51855 16.1046 5.51855 15.5419V7.58557L1.5 3.65002H19.1729Z"
      fill="currentColor"
    />
  </svg>
));

AgentTraceConversationCitation.displayName = 'AgentTraceConversationCitation';

export default AgentTraceConversationCitation;
