    ...
    <div className="flex flex-col gap-1 pt-4">
      ...
      {field.type === 'select' ? (
        ...
        <div className="flex h-10 items-center overflow-hidden rounded-xl bg-future-surface-overlay shadow-sm">
          ...
          />
          <button className="flex h-full w-[50px] items-center justify-center border-l border-future-border text-future-foreground-muted transition-colors hover:text-future-foreground">
...
export function PropertiesSimple({
  ...
  const defaultExpandedKeys = sections
    .filter((s) => s.defaultExpanded)
    .map((s) => s.label);

  return (
    <div className="flex h-full gap-4">
      {/* ── JSON Editor Drawer (opens to the left of the properties panel) ── */}
      {editorOpen && (
        <div className="flex h-full w-[480px] shrink-0">
      ...
      <div
        className={cn(
          'flex w-[360px] shrink-0 flex-col overflow-hidden rounded-2xl border border-future-border bg-future-surface-raised',
          className
        )}
      >
        {/* ── Header ────────────────────────────────────────────── */}
