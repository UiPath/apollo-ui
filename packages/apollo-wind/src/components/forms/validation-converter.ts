 ...
 */
function isNumberType(fieldType: FieldType, customValueType?: CustomValueType): boolean {
  if (fieldType === 'custom') return customValueType === 'number';
...
function applyNumberConstraints(
  ...
  if (!isNumberType(fieldType, customValueType)) {
    return schema;
  }
...
export function buildZodSchemaFromFields(
  ...
  for (const field of fields) {
    shape[field.name] = validationConfigToZod(field.validation, field.type, field.valueType);
  }
