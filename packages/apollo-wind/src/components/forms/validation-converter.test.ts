...
describe('validationConfigToZod', () => {
  ...
  describe('custom-field value shape', () => {
    ...
    // The string and array shapes reached their constraint helpers, but the number shape did
    // not: `applyNumberConstraints` only saw `fieldType`, so every numeric constraint was
    // silently dropped for a custom field and `{ min: 1 }` still accepted 0.
    it('honours valueType through buildZodSchemaFromFields, not just the single-field path', () => {
      // The helper is exported, so a consumer building a whole-form schema through it would
      // otherwise get z.any() for every typed custom field and no enforcement at all.
      ...
      expect(schema.safeParse({ entities: ['a'] }).success).toBe(true);
      expect(schema.safeParse({ entities: [] }).success).toBe(false);
    });

    it('applies numeric constraints to a declared number-shaped custom field', () => {
      const ranged = validationConfigToZod({ min: 1, max: 10 }, 'custom', 'number');
      expect(ranged.safeParse(5).success).toBe(true);
      expect(ranged.safeParse(0).success).toBe(false);
      expect(ranged.safeParse(11).success).toBe(false);

      const wholeOnly = validationConfigToZod({ integer: true }, 'custom', 'number');
      expect(wholeOnly.safeParse(3).success).toBe(true);
      expect(wholeOnly.safeParse(3.5).success).toBe(false);
    });
  });
