...
const OVERLAY_BACKDROP_HANDLER_KEYS = [
  'onClick',
  'onMouseDown',
  'onMouseUp',
  'onPointerDown',
...
function guardOverlayBackdropHandlers(
  overlayProps?: React.ComponentPropsWithoutRef<typeof DialogPrimitive.Overlay>
) {
  if (!overlayProps) return overlayProps;
  const guarded = { ...overlayProps };
  for (const key of OVERLAY_BACKDROP_HANDLER_KEYS) {
    const handler = overlayProps[key];
...
>(function DialogContent(
  ...
  const content = (
    <DialogOverlay
      {...guardOverlayBackdropHandlers(overlayProps)}
