...
import { surface, accent, foreground, border, ring } from './colors';
...
function TokenRow({
  ...
  return (
    <div className="flex items-center gap-6 border-b border-future-border-subtle py-3">
      ...
      <span className="w-52 shrink-0 text-xs text-future-foreground-subtle">
        {className}
      </span>
      {children}
    </div>
      ...
      <div className="flex flex-col gap-4">
        {Object.entries(surface).map(([key, twClass]) => (
          <TokenRow key={key} label={`surface.${key}`} className={twClass}>
            <div
              className={cn(
                'h-10 w-24 rounded-lg border border-future-border',
                twClass
              )}
            />
          </TokenRow>
