...
import { MaestroTemplate, Grid, GridItem } from './template-maestro';
...
function RightPanelCard({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-sm leading-5 text-future-foreground-muted">
        {description}
      </p>
      {actionLabel && (
...
function SamplePageContent() {
  return (
    <div className="flex flex-col gap-6">
      {/* Full-width card */}
      <FullWidthCard title="1 column" subtitle="Full width column" />
...
function ImagePlaceholder({
  ...
  return (
    <div
      className="flex items-center justify-center rounded-xl border border-future-border-subtle bg-future-surface text-xs text-future-foreground-muted"
      style={{ width, height }}
    >
      {label ?? `${width}×${height}`}
    </div>
...
const sampleTableData: TableRow[] = [
  { id: '1', name: 'Automation Alpha', status: 'Active', type: 'Process', lastModified: '2026-02-01', owner: 'Alice' },
  { id: '2', name: 'Bot Bravo', status: 'Inactive', type: 'Task', lastModified: '2026-01-28', owner: 'Bob' },
  { id: '3', name: 'Connector Charlie', status: 'Active', type: 'Integration', lastModified: '2026-02-03', owner: 'Charlie' },
  { id: '4', name: 'Dashboard Delta', status: 'Pending', type: 'Process', lastModified: '2026-01-15', owner: 'Diana' },
  { id: '5', name: 'Engine Echo', status: 'Active', type: 'Task', lastModified: '2026-02-05', owner: 'Eve' },
  { id: '6', name: 'Flow Foxtrot', status: 'Active', type: 'Integration', lastModified: '2026-01-20', owner: 'Frank' },
  { id: '7', name: 'Gateway Golf', status: 'Inactive', type: 'Process', lastModified: '2026-01-10', owner: 'Grace' },
  { id: '8', name: 'Hub Hotel', status: 'Pending', type: 'Task', lastModified: '2026-02-04', owner: 'Hank' },
];
...
const sampleTableColumns: ColumnDef<TableRow, unknown>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => <span className="text-future-foreground-muted">{row.getValue('lastModified')}</span>,
  },
  {
    accessorKey: 'owner',
    header: ({ column }) => <DataTableColumnHeader column={column} title="Owner" />,
    cell: ({ row }) => <span className="text-future-foreground-muted">{row.getValue('owner')}</span>,
  },
];
...
const transactionData: TransactionRow[] = [
  { id: 'TXN-001', customer: 'Liam Johnson', email: 'liam@example.com', type: 'Sale', status: 'Completed', amount: '$250.00', date: '2026-02-05', details: 'Enterprise license — annual plan' },
  { id: 'TXN-002', customer: 'Olivia Smith', email: 'olivia@example.com', type: 'Subscription', status: 'Completed', amount: '$150.00', date: '2026-02-04', details: 'Pro plan — monthly renewal' },
  { id: 'TXN-003', customer: 'Noah Williams', email: 'noah@example.com', type: 'Sale', status: 'Processing', amount: '$350.00', date: '2026-02-04', details: 'Team license — 5 seats' },
  { id: 'TXN-004', customer: 'Emma Brown', email: 'emma@example.com', type: 'Refund', status: 'Completed', amount: '-$45.00', date: '2026-02-03', details: 'Partial refund — unused credits' },
  { id: 'TXN-005', customer: 'James Jones', email: 'james@example.com', type: 'Sale', status: 'Completed', amount: '$89.00', date: '2026-02-03', details: 'Starter plan — annual' },
  { id: 'TXN-006', customer: 'Sophia Garcia', email: 'sophia@example.com', type: 'Payout', status: 'Pending', amount: '$1,200.00', date: '2026-02-02', details: 'Partner payout — January' },
  { id: 'TXN-007', customer: 'Benjamin Miller', email: 'ben@example.com', type: 'Subscription', status: 'Completed', amount: '$29.00', date: '2026-02-02', details: 'Basic plan — monthly renewal' },
  { id: 'TXN-008', customer: 'Mia Davis', email: 'mia@example.com', type: 'Sale', status: 'Failed', amount: '$450.00', date: '2026-02-01', details: 'Enterprise license — payment declined' },
  { id: 'TXN-009', customer: 'Lucas Wilson', email: 'lucas@example.com', type: 'Sale', status: 'Completed', amount: '$199.00', date: '2026-02-01', details: 'Pro plan — annual' },
  { id: 'TXN-010', customer: 'Charlotte Moore', email: 'charlotte@example.com', type: 'Subscription', status: 'Processing', amount: '$59.00', date: '2026-01-31', details: 'Team plan — monthly renewal' },
];
...
const transactionColumns: ColumnDef<TransactionRow, unknown>[] = [
  DataTableSelectColumn<TransactionRow>(),
  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-col">
        <span className="text-sm font-medium text-future-foreground">{row.getValue('customer')}</span>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <span className={`text-sm ${colors[status] ?? 'text-future-foreground-muted'}`}>{status}</span>;
    },
  },
  {
    accessorKey: 'amount',
...
const recentActivity = [
  { initials: 'LJ', name: 'Liam Johnson', action: 'Purchased Enterprise license', time: '2 min ago' },
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {dashboardStats.map((stat) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-future-foreground-muted">{stat.title}</span>
                ...
                {recentActivity.map((item, i) => (
                  ...
                  >
                    ...
                    <div className="flex flex-1 flex-col">
                      <span className="text-sm font-medium text-future-foreground">{item.name}</span>
                ...
                {monthlyGoals.map((goal) => {
                  ...
                  return (
                    <div key={goal.label} className="flex flex-col gap-2">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium text-future-foreground">{goal.label}</span>
